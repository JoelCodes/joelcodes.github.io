...
must_haves:
  ...
  key_links:
    - from: "src/components/ui/Button.astro"
      ...
      via: "CSS custom properties for colors and borders"
      pattern: "var\\(--color-"
    - from: "src/components/ui/Button.astro"
      to: "src/styles/global.css"
      via: "Border thickness token"
...
- Use design tokens from global.css:
  - Colors: --color-yellow, --color-turquoise, --color-magenta
  - Dark variants: --color-yellow-dark, --color-turquoise-dark, --color-magenta-dark
  - Borders: --border-neo-thick (4px), --border-neo (3px)
  - Text: --color-text-light, --color-text-dark
  - Font: --font-heading for button text
...
- Dark mode: Use --color-*-dark variants, adjust border color to --color-text-dark
...
- `grep 'var(--color-' src/components/ui/Button.astro` shows color tokens used
- `grep 'var(--border-neo' src/components/ui/Button.astro` shows border token used
- `grep 'var(--font-heading)' src/components/ui/Button.astro` shows font token used
- `grep 'var(--color-text-' src/components/ui/Button.astro` shows text color tokens used
  </verify>
  <done>
Button.astro exists with 3-layer structure, transform-based hover/active states, size variants, WCAG-compliant focus state, and verified usage of design tokens from global.css.
  </done>
</task>
...
6. Confirm design token usage in both components:
   - `grep -c 'var(--color-' src/components/ui/Button.astro` returns non-zero
   - `grep -c 'var(--border-neo' src/components/ui/Button.astro` returns non-zero
   - `grep -c 'var(--border-neo' src/components/ui/Card.astro` returns non-zero
</verification>
