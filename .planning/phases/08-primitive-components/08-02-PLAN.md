...
must_haves:
  truths:
    ...
    - "Human verifies dark mode changes component appearance correctly (Task 3 checkpoint)"
  artifacts:
  ...
  key_links:
    - from: "src/components/ui/Input.astro"
      ...
      via: "CSS custom properties for font"
      pattern: "var\\(--font-"
    - from: "src/components/ui/Input.astro"
      to: "src/styles/global.css"
      via: "CSS custom properties for text sizing"
      pattern: "var\\(--text-"
...
Verify design token usage for label styling:
- `grep 'var(--font-heading)' src/components/ui/Input.astro` confirms heading font on label
- `grep 'var(--text-sm)' src/components/ui/Input.astro` confirms text sizing token used
  </verify>
  <done>
Input.astro exists with WCAG 2.4.13 compliant focus ring (2px minimum, 3:1 contrast), error state handling, label support with design tokens, and HTML attribute forwarding.
  </done>
</task>
...
This page is temporary for development verification - it can be removed or hidden before production.
  ...
  <verify>
Verify component imports before visual check:
- `grep -c 'import.*Button' src/pages/component-demo.astro` returns non-zero
- `grep -c 'import.*Card' src/pages/component-demo.astro` returns non-zero
- `grep -c 'import.*Input' src/pages/component-demo.astro` returns non-zero
Run `npm run build` - no errors.
Run `npm run dev` and navigate to http://localhost:4321/component-demo
Visually inspect: all components render without errors.
  </verify>
  <done>
component-demo.astro page exists with verified imports of all Button, Card, and Input components, rendering all variants for visual inspection.
  </done>
</task>
...
2. Input.astro uses design tokens: --font-heading for label, --text-sm for sizing
3. component-demo.astro imports and renders all three primitives (verified via grep)
4. Build passes: `npm run build`
5. Human verifies visual appearance and dark mode behavior (Task 3 checkpoint)
</verification>
...
- Input label uses --font-heading and --text-sm design tokens
- component-demo.astro imports all three primitives (Button, Card, Input)
