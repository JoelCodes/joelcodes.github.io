...
<task type="auto">
  ...
  <verify>
    ...
    `npm run dev` starts successfully
    No console errors in browser
    Color inheritance test: Apply different text colors (text-yellow, text-turquoise, text-magenta) to elements with iso-glow and verify glows match those colors (yellow glow for text-yellow, turquoise glow for text-turquoise, magenta glow for text-magenta)
  </verify>
  <done>
    Isometric shadow/glow utilities added: iso-shadow-sm, iso-shadow, iso-shadow-lg with dark mode glow transformation, plus iso-glow-subtle, iso-glow, iso-glow-strong. Glow colors derive from element's own text color via currentColor.
  </done>
</task>
...
<task type="auto">
  ...
  <verify>
    ...
    Run `grep -E "iso-" src/pages/test-isometric.astro` to confirm iso- utility classes are actually used in the HTML (should show iso-container, iso-rotate, iso-shadow, iso-glow, iso-hover classes)
  </verify>
  <done>
    Test page created at /test-isometric demonstrating all isometric utilities in light and dark mode. Grep confirms iso- classes are applied to elements.
  </done>
</task>
