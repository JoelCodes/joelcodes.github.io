...
<task type="auto">
  ...
  <action>
    FIRST: Verify that only Footer.astro and contact.astro use lucide-static icons.
    Run: `grep -r "lucide-static" src/` to confirm no other files import from lucide-static.
    Expected result: Only two files (Footer.astro and contact.astro) should appear.
    If other files appear, update the migration plan to include them before proceeding.

    Then run `npm uninstall lucide-static && npm install @lucide/astro` to swap the icon library.
  ...
  <verify>
    `grep -r "lucide-static" src/` shows only Footer.astro and contact.astro (confirming complete migration scope)
    `npm list lucide-static` returns empty/error (uninstalled)
    `npm list @lucide/astro` shows version installed
    Build completes successfully
  </verify>
  <done>
    Package swap complete: lucide-static removed, @lucide/astro installed. Grep confirmed only 2 files used lucide-static.
  </done>
</task>
