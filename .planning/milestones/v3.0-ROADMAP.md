...
**Phases:** 33-41 (9 phases)
**Total Plans:** 45
**Git tag:** `v3.0`
**Milestone audit:** PASSED — 33/33 requirements, no critical blockers (`.planning/milestones/v3.0-MILESTONE-AUDIT.md`)

## Overview

Rebuilt the site in place to the "Joel Shinness Solutions" Figma brand — sea-cool palette, Fraunces + Hanken Grotesk, waveform mark, new Landing/Showcase IA with light + dark themes — replacing the neobrutalist design entirely while keeping the Astro/Tailwind infrastructure, CI, SEO plumbing, and blog content. Every visible-UI phase ended with a Figma-frame vs. rendered-page screenshot comparison gate (the discipline absent in v1.4). Design source of truth: Figma `1tg8wIPcvOVC5tPZ8pkGO2`.
...
**Decimal Phases:** None. v3.0 numbered 33–41 sequentially (23–32 consumed by the abandoned v2.0).

**Key Decisions:**

- Abandon the Crito/v2.0 direction unconsumed; adopt the new Figma "Joel Shinness Solutions" brand (sea-cool palette, Fraunces + Hanken Grotesk, waveform mark). v2.0 record preserved on unmerged branch `feature/phase-32-fidelity-sweep-handoff`.
- Rebuild in place — keep Astro 5 + Tailwind 4 infra, CI, SEO, blog content; replace tokens/components/layouts/pages wholesale.
- `--wl-*` token namespace throughout migration; old v1/v2 tokens kept alive until Phase 41 cleanup to prevent multi-page regressions.
- Design-fidelity screenshot gate enforced at every visible-UI phase (34–39) and as the formal milestone-close approval at Phase 41 (QUAL-03).
- Blog fully dev-only (link + pages out of prod builds and sitemap) — Joel reversed "URLs stable" in Phase 34; SEO value knowingly sacrificed until relaunch.
- System-only dark mode from the Figma dark mockup (`117:103`); no theme toggle (design toggle in Figma first).
- Service Web + Area Abbotsford pages built but dev-hidden (noindex + sitemap filter + PROD redirect guard); publish deferred to FUT.
- Real Calendly URL supplied mid-Phase-37 and shipped live via `BOOKING_URL` (resolved FUT-01).

**Issues Resolved:**

- Dark-mode gradient cascade bug (Phase 37 code review).
- Blog post LCP bug resolved structurally — post layout renders no top image.
- Phase 37 dev-toolbar test-selector gap (8/9 → 9/9) closed by scoping assertions to `main > section` (2026-07-17).
- ProjectCard `<details>` story-in-DOM for SEO indexing; `::details-content` progressive enhancement shared in global.css.

**Issues Deferred (tracked in v3.0-MILESTONE-AUDIT.md tech debt):**

- QUAL-02 Lighthouse ≥90 configured (/ and /404) but only scores in CI on push to main — auto-resolves on PR merge.
- `/projects/[slug]` + `/portfolio/[slug]` dynamic redirects omitted (Astro static-mode limit, D-02).
- `BOOKING_URL` duplicated as a literal in the `/contact` config redirect (sync-comment guarded).
- 4 Service Web FAQ answers = COPY GAP (not in Figma) — deferred to Joel (FUT).
- Phases 39 & 41 verified via gate summaries rather than `VERIFICATION.md` files (artifact-naming debt).

**Technical Debt Incurred:**

- Nyquist VALIDATION.md absent for Phases 36, 38, 39 (discovery-only; non-gating).
- Untracked `design/image-import.{png,jpg}` root duplicates remain (CLEAN-03 residue).

**Deferred future requirements (FUT-02..07):** publish Service Web / Area Abbotsford, additional service pages, blog back into public nav, real social links, real case study for the "Your Project Here" slot.
