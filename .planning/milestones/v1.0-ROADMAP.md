...
**Key Decisions:**
- Static site over CMS (Joel can edit code; simpler hosting)
- Astro + Tailwind v4 (modern stack, fast builds)
- Contact form over booking link (lower friction)
- Single portfolio project to start (ship faster)
- MDX for blog (rich content with code blocks)
- Homepage About section (removed separate /about page)
- Formspree for forms (no backend needed)

**Issues Resolved:**
- Removed /about nav link (page didn't exist, linked to homepage section)
- Fixed Lighthouse scores (achieved 92/90/100/100)

**Technical Debt:**
- Formspree form ID is placeholder (needs user config before deployment)
- WebP/AVIF optimization not implemented (lazy loading works, Lighthouse 92 acceptable)
- Social links in JSON-LD schema empty (user needs to provide real URLs)
