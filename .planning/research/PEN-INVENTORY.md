...
| **FAQ** | **b7Hgy** | **joel-only-no-crito-ref** | **/faq (per Plan 26-01)** | **5** | **flat:0, partial:0, factored:5** | **n/a (Phase 26 plan 26-01 — reconstructed)** | **OPEN-26-02 (now resolved)** |
...
| OPEN-26-02 | tooling | minor | **New (plan 26-01 Task 5).** `mcp__pencil__get_screenshot` returned blank-white image for any frame newly created within the active Pencil MCP session — FAQ page frame `b7Hgy`, its 4 sub-frames (`I4QJas`, `FswuE`, `eDbs7`, etc.), ref instances inside it (`MpVz3` to `G0wNOc`, `NQNB3` to `Hs5rc`), and an ephemeral test-probe frame. Pre-existing nodes (`G0wNOc`, `g9oRa5`, `cl8tt`) screenshot correctly the entire time. `batch_get` + `snapshot_layout` showed all data correct → confirmed rendering-pipeline-only quirk. **RESOLVED in-plan:** After an `Update("b7Hgy", {x, y})` position correction in Task 5, subsequent `get_screenshot` calls on the same nodes rendered correctly (Task 6 calibration spot-check screenshots produced full visual renders). Working theory: a stale render cache that clears on any node-property Update touching the affected subtree. | 26-02 (404 reconstruction) + 27+ (per-page phases) | **WORKAROUND CODIFIED in Plan 26-03 CALIBRATION-PROTOCOL.md:** if `get_screenshot` returns blank for a newly-created frame, call `Update(<frame>, {x: <current x>, y: <current y>})` (no-op position assignment to current values) and re-screenshot. If still blank, fall back to user-side verification in Pencil's actual editor at the calibration gate. |
...
**Phase 26 Variant Evidence outcome:** 17 rows from plan 26-00 + 12 rows from plan 26-01 = **29 rows total**. Per OPEN-23-13 dual-track: all literal hex/size/typography values appear here bound to their semantic alias counterparts; downstream consumers (Plan 26-02 404 + Phase 27 Thank-you/Contact + Phase 28 Blog + Phase 29 Projects + Phase 30 Design-system + Phase 31 Homepage) instance these section components without redrawing.

**Library count after Phase 26 plan 26-00:** `_Components / Sections` parent (g9oRa5) now hosts FOUR reusable section components: Header (G0wNOc) + Footer (Xs0Hs) [Phase 25] + CTA (Hs5rc) + NavBack (N1jo3i) [Phase 26]. Was 2 at end of Phase 25; now 4.

**Page-frame count after Phase 26 plan 26-01:** 1 reconstructed top-level page frame: `FAQ` (b7Hgy). 14 Crito-source raster page frames (cl8tt, w1m3x, DzqTm, etc.) remain unmodified. Plan 26-02 (404) will add a second reconstructed page frame.
